...
namespace Assembler.Logic.Commands
{
	class IDIV: Command
	{
		...
		{
			return Factory.Assemble1Arg(cmd, "IDIV", this.Argument1, 0x07);
		}

		public static IDIV Create(ILineInfo line)
		{
			if (line.NumberOfArguments != 1)
			{
				throw new Exceptions.ArgumentNumberException("IDIV", line.NumberOfArguments);
			}
			var cmd = new IDIV()
			{
			...
			{
				throw new Exceptions.ArgumentException("IDIV", line.TypeOfArgument(1));
			}
			return cmd;
		}
	}
