...
namespace Assembler.Logic.Commands
{
	class IMUL: Command
	{
		...
		public bool CheckArgumentSize()
		{
			...
			if (Argument1.IsWord && (Argument2.IsWord || Argument2 is Number))
			{
				return true;
			}
			throw new Exceptions.ArgumentSizeException("IMUL");
		}

		public Func<MemoryManager, byte[]> FactoryAssemble1(byte cmd)
		{
			return Factory.Assemble1Arg(cmd, "IMUL", this.Argument1, 0x05);
		}

		public Func<MemoryManager, byte[]> FactoryAssemble2(byte cmd, byte? prefix = null)
		{
			var iReg = (Argument1 as Register).Code;
			return Factory.Assemble2Args(cmd, "IMUL", this.Argument1, this.Argument2, iReg, prefix, true);
		}
