...
namespace Assembler.Logic.Commands
{
	class ADD: Command
	{
		...
		public bool CheckArgumentSize()
		{
			...
			throw new Exceptions.ArgumentSizeException("ADD");
		}
		
		public Func<MemoryManager, byte[]> FactoryAssemble(byte cmd)
		{
			return Factory.Assemble2Args(cmd, "ADD", this.Argument1, this.Argument2, 0x00);
		}

		public static ADD Create(ILineInfo line)
		{
			if (line.NumberOfArguments != 2)
			{
				throw new Exceptions.ArgumentNumberException("ADD", line.NumberOfArguments);
			}
			var cmd = new ADD()
			{
				...
				case Lexer.ArgumentType.Register:
					...
					switch (line.TypeOfArgument(2))	
					{
						case Lexer.ArgumentType.Register:
							...
							cmd.Assemble = cmd.FactoryAssemble(0x00);
							break;
						...
						case Lexer.ArgumentType.Indirect:
							...
							cmd.Assemble = cmd.FactoryAssemble(0x02);
							isMem = true;
							break;
					}
					break;
			...
			if (cmd.Assemble == null)
			{
				...
				throw new Exceptions.ArgumentException("ADD", line.TypeOfArgument(1), line.TypeOfArgument(2), lastCapture);
			}
			if (!isMem)
			{
