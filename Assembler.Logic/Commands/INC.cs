...
namespace Assembler.Logic.Commands
{
	class INC: Command
	{
		...
		{
			return Factory.Assemble1Arg(cmd, "INC", this.Argument1, 0x00);
		}

		public static INC Create(ILineInfo line)
		{
			if (line.NumberOfArguments != 1)
			{
				throw new Exceptions.ArgumentNumberException("INC", line.NumberOfArguments);
			}
			var cmd = new INC()
			{
			...
			{
				throw new Exceptions.ArgumentException("INC", line.TypeOfArgument(1));
			}
			return cmd;
		}
	}
